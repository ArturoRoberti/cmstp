...
marker_in_file() {
  : '
    Check if a CSTP marker exists in a logfile.
    Args:
      $1 - logfile (string)
      $2 - marker (string)
    '
  ...
  if grep -qE "CSTP START[[:space:]]+$marker" "$logfile"; then
...
log_to_file() {
  ...
  # Write start marker
  echo -e "\n${hashes} CSTP START ${marker} ${hashes}\n" >>"$logfile"
  ...
  # Write end marker
  echo -e "\n${hashes} CSTP END ${marker} ${hashes}" >>"$logfile"
}

# TODO: Adapt to more than just CSTP markers (?), e.g. loki shell markers
remove_marker_from_file() {
  : '
    Remove all sections wrapped by a specific CSTP marker from a logfile.
    Args:
      $1 - logfile (string)
      $2 - marker (string)
    '
  ...
  # Use sed to delete from CSTP START <marker> to CSTP END <marker>
  sed -i "/#* CSTP START[[:space:]]\+$marker #*/,/#* CSTP END[[:space:]]\+$marker #*/d" "$logfile"
}
