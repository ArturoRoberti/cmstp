...
add_graphics_drivers_ppa() {
  ...
  if [[ "$prime_select" == "true" ]]; then
    ...
    sudo tee /etc/apt/preferences.d/cstp-nvidia-driver-pin >/dev/null <<'EOF'
...
configure_apt_repositories_cuda() {
  ...
  if [[ -f "/etc/apt/preferences.d/cstp-nvidia-driver-pin" ]]; then
    sudo rm -f /etc/apt/preferences.d/cstp-nvidia-driver-pin
  fi
